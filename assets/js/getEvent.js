...
function placeEvent(event, setDateCallback) {
    var jcalData = ical_js_1.parse(event);
    var vcalendar = new ical_js_1.Component(jcalData);
    var vevent = vcalendar.getFirstSubcomponent('vevent');
    var dstart = vevent.getFirstPropertyValue('dtstart');
    var dt = new Date(Date.parse(dstart));
    ...
    var prettyDate = dt.toLocaleString('en-US', options);
    setDateCallback(prettyDate);
}
// Get an ics calendar from url. 
// Call placeEvent to parse and pass on to callback.
function getEvent(icsUrl, setDateCallback) {
    fetch(icsUrl)
        .then(function (response) { return response.text(); })
        .then(function (response) { return placeEvent(response, setDateCallback); })
        .catch(function (response) { return setDateCallback("Error fetching event date."); });
}
...
},{"ical.js":2}],2:[function(require,module,exports){
